...
import { getAdminFacilities, deleteAdminFacility } from '@/lib/api';
import {
  Sliders, Plus, Search, RefreshCw, Edit3, Trash2, CheckCircle2, AlertCircle
} from 'lucide-react';
...
export default function AdminFacilitiesPage() {
  ...
  const fetchFacilities = async () => {
    ...
    try {
      ...
      const res = await getAdminFacilities(token, params);
  ...
  const handleDelete = async (item: any) => {
    if (!confirm(`Are you sure you want to delete facility "${item.name}"?`)) return;
    const token = localStorage.getItem('pocono_admin_token');
    if (!token) return;
    ...
    setSuccessAlert('');
    try {
      await deleteAdminFacility(item.id, token);
      setSuccessAlert(`Facility "${item.name}" deleted successfully!`);
      setTimeout(() => setSuccessAlert(''), 3000);
      fetchFacilities();
    } catch (err: any) {
      setErrorAlert(err.message || 'Error deleting facility');
    }
  };
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-[#e5e7eb] pb-6">
        <div>
          <span className="text-[#f15e75] text-xs font-bold uppercase tracking-wider block mb-1">
            PostgreSQL Property Facilities
          </span>
          <h1 className="text-3xl font-extrabold text-[#2b2b2b]">Property Facilities</h1>
        </div>
      ...
      <div className="bg-white p-4 border border-[#e5e7eb] rounded-md shadow-sm flex justify-between items-center">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search facility name, slug..."
            value={searchTerm}
        ...
        <span className="text-xs text-[#6b7280] font-bold">{facilities.length} Facilities Total</span>
      </div>

      {/* Table */}
      {loading ? (
        <AdminLoader variant="table" message="Loading Facilities Collection..." />
      ) : facilities.length === 0 ? (
        <div className="p-12 bg-white border border-[#e5e7eb] rounded-md text-center space-y-2 shadow-sm">
          <p className="text-[#2b2b2b] font-bold">No facilities found matching search query.</p>
        </div>
      ) : (
