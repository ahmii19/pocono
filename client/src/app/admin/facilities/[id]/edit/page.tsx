...
import { getAdminFacilityById, updateAdminFacility } from '@/lib/api';
import { ArrowLeft, Save, Sliders, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function AdminEditFacilityPage() {
  ...
  const fetchFacility = async () => {
    ...
    try {
      const res = await getAdminFacilityById(facilityId, token);
      const f = res.data;
      if (f) {
        setName(f.name || '');
        setSlug(f.slug || '');
      ...
      setError(e.message || 'Failed to load facility details');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateAdminFacility(facilityId, { name, slug }, token);
      setSuccess('Facility updated successfully in PostgreSQL!');
      setTimeout(() => setSuccess(''), 3000);
    } catch (err: any) {
      setError(err.message || 'Error updating facility');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (initialLoading) {
    return <div className="p-8 text-center text-[#4f5962] text-sm font-medium">Loading facility details...</div>;
  }
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="flex justify-between items-center border-b border-[#e5e7eb] pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-[#2b2b2b]">Edit Facility</h1>
            <p className="text-xs text-[#6b7280]">Updating PostgreSQL Facility ID: {facilityId}</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-[#e5e7eb] rounded-md shadow-sm p-6 space-y-4 text-xs text-[#4f5962]">
        ...
        <div className="pt-4 border-t border-[#e5e7eb] flex justify-end">
          ...
          >
            ...
            <span>{loading ? 'Saving Changes...' : 'Save Facility Changes'}</span>
          </button>
        </div>
      </form>
    </div>
