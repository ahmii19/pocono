...
import { createProperty, getCities, getCommunities, getPropertyTypes, getAdminUsers } from '@/lib/api';
...
export default function AdminNewPropertyPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [cRes, commRes, tRes, uRes] = await Promise.all([
        ...
        getCommunities(),
        getPropertyTypes(),
        token ? getAdminUsers(token) : Promise.resolve({ data: [] })
      ]);
      setCities(cRes.data || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const createdPropertyId = res?.data?.id;

      setSuccess('Property created successfully! Redirecting to photo management gallery...');

      setTimeout(() => {
        if (createdPropertyId) {
          router.push(`/admin/properties/${createdPropertyId}/media`);
        } else {
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex justify-between items-center border-b border-[#e5e7eb] pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-[#2b2b2b]">Create New Property</h1>
            <p className="text-xs text-[#6b7280]">Add a new vacation rental via 4-step wizard</p>
          </div>
        </div>
      </div>
      ...
      {/* Property Creation Wizard Container */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Pocono Pine Chalet"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-[#f8fafc] border border-[#e5e7eb] text-[#2b2b2b] rounded-md px-3 py-2 text-xs font-semibold focus:outline-none focus:border-[#f15e75]"
                  >
                    <option value="">Default Admin Host</option>
                    {hosts.map((h) => (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. 123 Lake Harmony Rd"
                    className="w-full bg-[#f8fafc] border border-[#e5e7eb] text-[#2b2b2b] rounded-md px-3 py-2 text-xs font-medium focus:outline-none focus:border-[#f15e75]"
                  />
                </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="p-4 bg-gray-50 border border-[#e5e7eb] rounded-md space-y-2 text-xs">
                <h4 className="font-extrabold text-[#2b2b2b]">Ready to Create Property</h4>
                <p className="text-gray-600 leading-relaxed">
                  Review summary: <strong>{title}</strong> (${nightlyPrice}/night, {maxGuests} Guests). Submitting will create the property in PostgreSQL and allow photo uploads next.
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-between pt-4 border-t border-[#e5e7eb]">
              ...
              >
                ...
                <span>{loading ? 'Creating Property...' : status === 'PUBLISHED' ? 'Save & Publish Property' : 'Save as Draft Property'}</span>
              </button>
            </div>
          </form>
        )}
