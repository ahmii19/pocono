...
import { getAdminPropertyTypeById, updateAdminPropertyType } from '@/lib/api';
import { ArrowLeft, Save, Home, AlertCircle, CheckCircle2 } from 'lucide-react';

export default function AdminEditPropertyTypePage() {
  ...
  const fetchPropertyType = async () => {
    ...
    try {
      const res = await getAdminPropertyTypeById(typeId, token);
      const pt = res.data;
      if (pt) {
        setName(pt.name || '');
        setSlug(pt.slug || '');
      ...
      setError(e.message || 'Failed to load property type details');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateAdminPropertyType(typeId, { name, slug }, token);
      setSuccess('Property type updated successfully in PostgreSQL!');
      setTimeout(() => setSuccess(''), 3000);
    } catch (err: any) {
      setError(err.message || 'Error updating property type');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (initialLoading) {
    return <div className="p-8 text-center text-[#4f5962] text-sm font-medium">Loading property type details...</div>;
  }
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="flex justify-between items-center border-b border-[#e5e7eb] pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-[#2b2b2b]">Edit Property Type</h1>
            <p className="text-xs text-[#6b7280]">Updating PostgreSQL Property Type ID: {typeId}</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-[#e5e7eb] rounded-md shadow-sm p-6 space-y-4 text-xs text-[#4f5962]">
        ...
        <div className="pt-4 border-t border-[#e5e7eb] flex justify-end">
          ...
          >
            ...
            <span>{loading ? 'Saving Changes...' : 'Save Type Changes'}</span>
          </button>
        </div>
      </form>
    </div>
